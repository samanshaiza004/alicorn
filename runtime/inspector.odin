...
inspect :: proc(rt: ^Runtime) -> string {
	...
	fmt.sbprintf(&sb, "frame: %d built=%d idle=%d regions-skipped=%d adjacency-rebuilds=%d\n", rt.stats.frame, rt.stats.frames_built, rt.stats.idle_frames, rt.stats.regions_skipped, rt.stats.adjacency_rebuilds)
	for id in rt.order {
		node, ok := rt.nodes[id]
		if !ok { continue }
