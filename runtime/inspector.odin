...
inspect :: proc(rt: ^Runtime) -> string {
	...
	fmt.sbprintf(&sb, "focused: %d selected: %d\n", rt.focused, rt.selected)
	...
	for id in rt.order {
		...
		fmt.sbprintf(&sb, "  selected=%t hovered=%t pressed=%t reason: %s\n", node.selected, node.hovered, node.pressed, node.last_reason)
	}
	if rt.hard_error {
