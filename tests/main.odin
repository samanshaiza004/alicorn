...
test_checkbox_and_slider_controls :: proc(state: ^Test_State) {
	...
	if checkbox_id != 0 && slider_id != 0 {
		...
		expect(state, !alicorn.activate_focused(&rt, .Enter), "Enter does not toggle a focused checkbox")
		_, _, check_change, _ = render_foundation_controls(&rt, false, 0.5)
		expect(state, !check_change.changed && !check_change.value, "checkbox remains unchanged after Enter")
		expect(state, alicorn.activate_focused(&rt, .Space), "focused checkbox accepts Space activation")
		_, _, check_change, _ = render_foundation_controls(&rt, false, 0.5)
		...
		_, slider_id, _, _ = render_foundation_controls(&rt, check_change.value, 0.6, step=0.3)
		expect(state, alicorn.focus(&rt, slider_id), "slider with non-divisible range accepts focus")
		expect(state, alicorn.set_focused_slider_bound(&rt, .Maximum), "End is consumed by a focused slider")
		_, _, _, slider_change = render_foundation_controls(&rt, check_change.value, 0.6, step=0.3)
		expect(state, slider_change.changed && slider_change.value == 1, "End reaches the exact maximum even when it is off the step grid")
		expect(state, alicorn.set_focused_slider_bound(&rt, .Minimum), "Home is consumed by a focused slider")
		_, _, _, slider_change = render_foundation_controls(&rt, check_change.value, 1, step=0.3)
		expect(state, slider_change.changed && slider_change.value == 0, "Home reaches the exact minimum")
...
test_keyboard_focus_and_activation :: proc(state: ^Test_State) {
	...
	expect(state, alicorn.activate_focused(&rt, .Space), "focused button must also accept Space activation")
	_, clicked = render_canonical_button(&rt)
	expect(state, clicked, "Space activation reaches the public button API")
	_, clicked = render_canonical_button(&rt)
