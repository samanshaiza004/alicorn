/*
UAX #29 sentence-break conformance harness.

Runs `itemize.sentence_iter_next` against every test case in
`tools/ucd/SentenceBreakTest.txt` and reports pass / mismatch
counts. Format mirrors the grapheme + word harnesses.

Run:
    odin run tools/sb_conformance.odin -file -o:speed
*/
package main
...
main :: proc() {
	data, err := os.read_entire_file_from_path("tools/ucd/SentenceBreakTest.txt", context.allocator)
	...
	for line in strings.split_lines_iterator(&iter) {
		...
		text := string_from_runes_sb(runes[:])
		got_breaks := make([dynamic]int, 0, 16, context.temp_allocator)
		it := itemize.sentence_iter_make(text)
		for {
			lo, hi, ok := itemize.sentence_iter_next(&it)
			if !ok { break }
			_ = lo
	...
	fmt.printfln("UAX #29 SentenceBreakTest: %d / %d passed (%.2f%%)  mismatches=%d", passed, total, pct, mismatches)
}

tokenize_sb :: proc(s: ^string) -> [dynamic]string {
...
string_from_runes_sb :: proc(rs: []rune) -> string {
	b := strings.builder_make(context.temp_allocator)
	for r in rs { strings.write_rune(&b, r) }
	return strings.to_string(b)
}

utf8_rune_size_sb :: proc(r: rune) -> int {
	switch {
